...
terraform {
  ...
  backend "s3" {
    ...
    bucket = "jzohn-remote-state"          # Nome do bucket em bucket.tf que armazenara o arquivo de state   
    key    = "aws-instance/terraform.tfstate"  # Path do arquivo de state para o exemplo de criação da VM EC2 do curso
    region = "sa-east-1"
  }
}
